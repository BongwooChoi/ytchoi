...
def summarize_with_gemini(transcript, video_title="YouTube 영상"):
    ...
    try:
        ...
        # 향상된 프롬프트 생성
        prompt = f"""
다음은 "{video_title}" YouTube 영상의 자막입니다. 고품질 요약을 작성해주세요.

📹 영상 자막:
{transcript}

📝 요약 작성 가이드라인:
1. **핵심 메시지 파악**: 영상의 주요 목적과 핵심 내용을 정확히 파악
2. **구조화된 요약**: 논리적 순서로 내용을 정리 (도입-전개-결론)
3. **구체적 정보 포함**: 숫자, 고유명사, 중요한 팩트는 정확히 기재
4. **실용적 가치**: 시청자가 얻을 수 있는 실질적 정보나 인사이트 강조

📋 요약 형식:
- 첫 문장: 영상의 주제와 목적
- 중간 문장들: 핵심 내용과 주요 포인트들
- 마지막 문장: 결론이나 시청자에게 주는 가치

⚠️ 제외사항:
- 불필요한 감탄사, 추임새, 반복 표현
- "안녕하세요", "구독", "좋아요" 등 일반적인 인사말
- 광고나 홍보성 멘트

한국어로 정확하고 유용한 요약을 작성해주세요:
"""
